...
class InstrumentsControllerTest < ActionController::TestCase
  ...
  test 'should get index without edit buttons when logged in' do
    ...
    objects = assigns(:instruments)
    assert_not_nil objects
    objects.each do |object|
      assert_select 'a[href=?]', edit_instrument_path(object), count: 0
      assert_select 'a[href=?]', instrument_path(object),
                    text: 'Destroy',
                    count: 0
    end
  end
  ...
  test 'should get index with edit buttons when librarian' do
    ...
    assert_not_nil objects
    objects.each do |object|
      assert_select 'a[href=?]', edit_instrument_path(object)
      assert_select 'a[href=?]', instrument_path(object),
                    text: 'Destroy'
    end
  end
